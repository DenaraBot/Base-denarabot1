...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    zeroyt7.on('open', () => {
        success('2', 'Terhubung Jangan Lupa Subscribe Denara Bot')
    })
...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        wel = `Halo @${anu_user} \nWelcome In ${mdata.subject} \nJangan Lupa Follow Sosial Media Ownerku\n• Youtube : https://youtube.com/channel/UCcOCTySo6X1K5tziHZxq7Qw\n• Instagram : https://instagram.com/denarabot\n• Tiktok : https://tiktok.com/@kaptenmt\n• Github : https://github.com/Denarabot`
        ...
        sendButImage(mdata.id, wel, "©Created : Denara Bot", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, out, "©Created : Denara Bot", buff, but)
      }
